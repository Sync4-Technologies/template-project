...
- **Fast lane para tarefa trivial** (2.3): fix de ≤20 linhas em ≤2 arquivos, com teste já cobrindo o caminho, sem tocar contrato/migration/authz/dependência/flag e sem mudar comportamento além do defeito → **engineer + CR numa passada**, pulando o QA-define. Critério objetivo de 4 itens, todos obrigatórios; o que NÃO se dispensa é gate local verde, CR e a prova de execução (o teste citado falha antes e passa depois, colado no PR). Antes, um fix de 5 linhas com o teste já falhando percorria 4+ spawns — a matriz de autonomia gradua decisões, não cerimônia. Exceção registrada no DoD (squad-core §K)
- **"Feature crítica" ganha materialidade** (2.4): o critério deixa de ser lista de temas e passa a ser tabela de eixos com contra-exemplo em cada um — authz é regra de acesso (não login que só cria sessão); dados pessoais são os que identificam uma **pessoa real** fora da sessão (não ID de sessão nem telemetria anônima). Desempate: se você não escreve "se isso falhar, [quem] perde [o quê]", não é crítica. Quando tudo é crítico, o SE vira carimbo. A lista duplicada no ADR-003 virou ponteiro para a fonte única
- **M0 propõe a própria delegação de merge** (2.5): o marco só-de-contrato existe para desbloquear paralelismo; esperar uma aprovação de merge separada devolve o bloqueio que ele veio remover
- **Em modo enforce, `git commit && git push` que bloqueia (exit 2) descarta o reminder de memória** do ramo de commit: o bloqueio é a informação que importa, e o protocolo de hook não combina stdout de contexto com exit code de bloqueio. Coberto por teste.
- **Casos de comportamento do hook no CI** (`scripts/ci/pre-bash-cases.sh`, chamado pelo job `hooks-smoke`): **35 asserções**, cada regra testada **nos dois sentidos** — o que ela deve pegar e o que ela não pode pegar (UP-26). Antes o `hooks-smoke` só fazia `bash -n` nos hooks de push/commit: nenhum comportamento era verificado.
- **Achados da revisão do PR #67, corrigidos antes do merge** (o CR independente rejeitou a primeira versão com 2 MAJOR):
  - `--force-with-lease` estava na lista de flags com valor **separado** — ela só aceita `=<ref>`. O parser engolia o *remote*, os refspecs saíam vazios e `git push --force-with-lease origin outra-branch` disparava o aviso da UP-01; em enforce, push legítimo bloqueado. **O teste da UP-02 passava pelo motivo errado** (usava `main`, que cai no fallback "refspec vazio = branch atual"): é a UP-26 violada dentro do arquivo que a cita. O caso adversarial agora usa outra branch.
  - O refresh em background rodava **depois** do aviso, então o único caminho que atualizava o cache era o que não avisava: estado terminal cacheado se auto-perpetuava por 24h (PR reaberto, ou PR novo na mesma branch, avisando sem chance de se corrigir — em enforce, todo push bloqueado). O refresh passou para antes de qualquer saída.
  - `+main`, `refs/heads/main` e `HEAD:main` não eram reconhecidos como a branch atual (aviso sumia); `:outra-branch` (deleção remota) caía no fallback e avisava; `--mirror` era tratado como tag apesar de empurrar a branch atual.
  - Cache corrompido matava o hook em `set -u` (`unbound variable`) **antes do check do marcador** — o aviso que deveria sair sumia junto. Agora: `head -1` + `cut -s` + validação numérica, e `.tmp` com nome único (invocações concorrentes na mesma worktree geravam o cache de duas linhas que causava isso).
  - Fast lane: a via "sem caminho executável" exigia uma prova de execução impossível, o que a tornava letra morta; e o texto dizia "substitui o QA-define" enquanto alterava o item de QA-**valida** do DoD. Agora são duas vias explícitas (2a com teste, 2b sem caminho executável), cada uma com a evidência que satisfaz o DoD.
