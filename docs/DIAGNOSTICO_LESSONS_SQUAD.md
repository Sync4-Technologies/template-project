...
>
> **STATUS: P0 APLICADO em 2026-07-03** (temas A, B, C, I, J, K — commit `5f202e7`). **P1 APLICADO em 2026-07-03** (temas D, E, F, L — commit `07638ed`). **P2 APLICADO em 2026-07-03** (tema G: skill `/squad-deploy-preflight` criada + padrões PaaS/Docker no devops + gate docker fiel no self-review; tema H: stack-conventions nodejs [NODE_ENV matrix, NestJS SWC/eslint-DI, lockfile, HTTP outbound] e react [server-only env, NEXT_PUBLIC build-time] + CR grep SSR + architect governança de contratos + QA fixtures realistas). **TEMA M APLICADO em 2026-07-04 via plugin Claude Code** (decisão do usuário: plugin direto, sem script intermediário): `plugin/` = fonte canônica (15 skills incl. nova `/squad-init`, 3 hooks com `${CLAUDE_PLUGIN_ROOT}`, template completo), `.claude-plugin/marketplace.json` na raiz (marketplace `pdati`), versionamento SemVer + `SQUAD_VERSION` por projeto, regra anti-drift no README. Layout legado (clone) mantido até migração dos projetos ativos. **Diagnóstico 100% aplicado.**
